...
function searchFilter(page_num) {
    ...
    var nama = $('#nama').val();
    var lokasi_penugasan = $('#lokasi_penugasan').val();
    var nip = $('#nip').val();

    var sortBy = $('#sortBy').val();
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>index.php/penugasan/ajaxPaginationData/'+page_num,
        data:'page='+page_num+'&nama='+nama+'&nip='+nip+'&lokasi_penugasan='+lokasi_penugasan+'&sortBy='+sortBy,
        beforeSend: function () {
            $('.loading').show();
        },
...
function reportFilter(page_num) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>index.php/penugasan/report/'+page_num,
        data:'page='+page_num+'&nama='+nama+'&nip='+nip+'&lokasi_penugasan='+lokasi_penugasan+'&sortBy='+sortBy,
        datatype: 'excel',
        beforeSend: function () {
            $('.loading').show();
        ...
        <div class="card-header">
           <i class="fa fa-align-justify"></i> <?= $box_title ?>
            <div class="float-xs-right">
                <?=anchor('penugasan/tambah/', '<i class="icon-note"></i> Add', array('class' => 'btn btn-sm'));?>
            </div>
        </div>
        <div class="card-block">
            <div class="pading5">
                <form action="<?= $form_action ?>" method="post" class="form-inline">
                    ...
                    <div class="form-group">
                        <input type="text" id="lokasi_penugasan" name="lokasi_penugasan" class="form-control" placeholder="Lokasi Penugasan" <?= ($level == "2") ? "hidden" : "" ?>>
                    </div>
                    <div class="form-group">
