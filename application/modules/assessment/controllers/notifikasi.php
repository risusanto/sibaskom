...
class Notifikasi extends MY_Controller {
	...
	/*
	  ...
	  */      
		...
		$expired = $this->assessment_m->get(['waktu_alert' => date('Y-m-d')]);
		...
		if($countExp > 0){
			...
			$messg .= "Ini adalah notifikasi SIMOPS (Sistem Informasi Monitoring Data Bidang Pengembangan SDM) PT PLN (Persero). \n";
			   ...
			   {
			   		$messg .= "\nNama: $row->nama\nNIP: $row->nip\nMasa Berlaku Assessment Berakhir pada: ".date("d-m-Y", strtotime($row->tanggal_akhir_berlaku))."\n";
			   }

			$messg .= "\nMasa berlaku assessment akan berakhir\n\nBest Regard\nAdmin SIMOPS";
	   	$messgs = "<b>tes</b>";
